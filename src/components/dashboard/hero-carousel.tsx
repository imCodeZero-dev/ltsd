...
export function HeroCarousel({ slides }: { slides: HeroSlide[] }) {
  ...
  return (
    <div className="relative">
      ...
      >
        {/* All slides stacked — only active one is visible via opacity */}
        <div className="relative min-h-56 md:min-h-96">
          {slides.map((slide, i) => (
            <div
              key={slide.slug}
              className={`${i === 0 ? "relative" : "absolute inset-0"} transition-opacity duration-500 ease-in-out ${
            ...
            >
                  ...
                  {slides.map((_, j) => (
                    <button
                      ...
                      className={`rounded-full transition-all duration-300 ${
                        j === active ? "w-2.5 h-2.5 bg-badge-bg" : "w-2 h-2 bg-border-mid"
                      }`}
                    />
                  ))}
                </div>
              ...
              {slide.discountPercent > 0 && (
                <div
                  className="absolute top-3 right-4 md:top-5 md:right-10 z-10 w-9 h-9 md:w-12 md:h-12 rounded-full flex items-center justify-center text-white text-2xs md:text-xs font-bold shadow"
