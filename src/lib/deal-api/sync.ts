...
export async function syncLightningDeals(): Promise<{ synced: number; errors: string[]; expired: number; categorized: number }> {
  ...
  try {
    // Cap at 500 per run (5 Keepa /product calls × 100 ASINs each, ~3-5s total).
    // With lightning sync running every 6 hours (changed 2026-08-19 from
    // every 2 hours), backlog clears within a few cycles.
