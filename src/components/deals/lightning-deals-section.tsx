...
export function LightningDealsSection({ deals, watchlistMap }: Props) {
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-lg bg-badge-bg/10 flex items-center justify-center">
            <Zap className="w-4 h-4 text-badge-bg" />
          </div>
          <div>
            <h2 className="type-section-title">Lightning Deals</h2>
            <p className="text-xs text-body mt-0.5">Limited stock, real countdown — grab before they're gone</p>
          </div>
        </div>
      ...
      >
        {deals.map((deal) => (
          <div key={deal.id} className="shrink-0 w-56 sm:w-64">
